...
namespace Foundatio.Repositories.Advanced
{
    ...
    public interface IAdvancedSearchableRepository<T> : IAdvancedRepository<T>, ISearchableRepository<T> where T : class, IIdentity, new() { }

    public static class RepositoryExtensions {
        public static Task PatchAsync<T>(this IAdvancedRepository<T> repository, Id id, IPatchOperation operation, CommandOptionsDescriptor<T> options = null) where T : class, IIdentity, new() {
            return repository.PatchAsync(id, operation, options.Configure());
        }

        public static Task PatchAsync<T>(this IAdvancedRepository<T> repository, Ids id, IPatchOperation operation, CommandOptionsDescriptor<T> options = null) where T : class, IIdentity, new() {
            return repository.PatchAsync(id, operation, options.Configure());
        }
